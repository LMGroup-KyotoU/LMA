...
import lma.env.tasks.humanoid_amp_task as humanoid_amp_task
...
class HumanoidTennis2(humanoid_amp_task.HumanoidAMPTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
        ...
        # # NOTE: for check eta effects
        # self._target_min = torch.FloatTensor(cfg['env'].get('target_min', [-9.5, 2.0, 0]))
        # self._target_max = torch.FloatTensor(cfg['env'].get('target_max', [-9.5, 2.0, 0]))
        ...
        self.launch_to_me_prob = cfg["env"].get('launch_to_me_prob', 0.5)
    ...
    def _update_camera(self):
        ...
        # for back of opponent
        new_cam_target = gymapi.Vec3(-12.0, 1., 1.)
        new_cam_pos = gymapi.Vec3(-16.0, 1, 3.)

        # for front of opponent
        # new_cam_target = gymapi.Vec3(-12.0, 0., 1.)
        # new_cam_pos = gymapi.Vec3(-16.0, 3., 3.)
        ...
        # # for front of agent
        # new_cam_target = gymapi.Vec3(12.0, 0., 1.)
        # new_cam_pos = gymapi.Vec3(7.0, 3., 3.)

        # # for all court
        # new_cam_target = gymapi.Vec3(0.0, 0., 1.)
        # new_cam_pos = gymapi.Vec3(0.0, -15., 3.)
    ...
    def _build_predefined_tensors(self):
        ...
        self._launch_prob = torch.zeros(self.num_envs, device=self.device, dtype=torch.float) + self.launch_to_me_prob
        self.launch_dir = torch.bernoulli(self._launch_prob)

        self.win_buf = torch.zeros(self.num_envs, device=self.device, dtype=torch.int)
        ...
        # for record
        self.ball_pos_buf = torch.zeros((0, self.num_envs, 3), device=self.device, dtype=torch.float)
        self.ball_vel_buf = torch.zeros((0, self.num_envs, 3), device=self.device, dtype=torch.float)
        self.reset_env_buf = torch.zeros((0, self.num_envs), device=self.device, dtype=torch.float)
    ...
    def set_initial_root_state(self):
        for i in range(self.num_agents):
            ...
            # initial_humanoid_root_states[..., 0] = self._baseline if i == 0 else -1 * self._baseline
    ...
    def _compute_reward(self, actions):
        ...
        if self.record_ball_pos:
            self.ball_pos_buf = torch.cat([self.ball_pos_buf, ball_pos.unsqueeze(0)])
            self.ball_vel_buf = torch.cat([self.ball_vel_buf, ball_vel.unsqueeze(0)])
    ...
    def save_ball_pos(self):
        ball_pos = self.ball_pos_buf.detach().cpu().numpy()
        ball_vel = self.ball_vel_buf.detach().cpu().numpy()
        reset_env = self.reset_env_buf.detach().cpu().numpy()
        np.savez('ball_position.npz', ball_pos=ball_pos, ball_vel=ball_vel, reset_env=reset_env)
    ...
    def _compute_reset(self):
        ...
        if self.record_ball_pos:
            self.reset_env_buf = torch.cat([self.reset_env_buf, self.reset_buf.unsqueeze(0)])
    ...
    def post_physics_step(self):
        ...
        if self.compete_reward:
            ...
            if len(terminate_env_ids > 0):
                ...
                me_reward, op_reward, me_win, op_win = compute_compete_rewards(
                    self._ball_mycourt_contact_buf[terminate_env_ids], self._ball_othercourt_contact_buf[terminate_env_ids], terminate_env_ids,
                    self._contact_forces_list, self._contact_body_ids,
                                                   self._rigid_body_pos_list, self._termination_heights, self.num_agents,
                                                   self.launch_dir[terminate_env_ids])
                self.rew_buf[terminate_env_ids] += me_reward
                self.rew_buf[self.num_envs+terminate_env_ids] += op_reward

                self.win_buf[terminate_env_ids] += me_win * 1
                self.win_buf[terminate_env_ids] += op_win * -1
...
def compute_compete_rewards(ball_mycourt_contact_buf, ball_othercourt_contact_buf, terminate_env_ids,
                            contact_buf_list, contact_body_ids, rigid_body_pos_list, termination_heights, num_agents, launch_dir):

    me_win = torch.where(launch_dir > 0, ball_othercourt_contact_buf >= ball_mycourt_contact_buf, ball_othercourt_contact_buf > ball_mycourt_contact_buf)
    op_win = torch.where(launch_dir > 0, ball_othercourt_contact_buf < ball_mycourt_contact_buf, ball_othercourt_contact_buf <= ball_mycourt_contact_buf)
    both_lose = torch.logical_and(ball_othercourt_contact_buf == 0, ball_mycourt_contact_buf == 0)

    me_reward = me_win * 100 - op_win * 100 - both_lose * 100
    op_reward = op_win * 100 - me_win * 100 - both_lose * 100

    return me_reward, op_reward, me_win, op_win
...
def compute_humanoid_reset(reset_buf, progress_buf, contact_buf_list, contact_body_ids, rigid_body_pos_list,
    ...
    if (enable_early_termination):
        ...
        # NOTE: comment out the following two lines for recording ball pos
