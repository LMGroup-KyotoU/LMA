...
class HumanoidPP2(humanoid_amp_task.HumanoidAMPTask):
    ...
    def _update_camera(self):
        ...
        new_cam_target = gymapi.Vec3(-0.37, 0., 1.)
        new_cam_pos = gymapi.Vec3(-3., 0, 2.5)
        ...
        # new_cam_pos = gymapi.Vec3(0.0, -2.5, 1.5)
    ...
    def _build_ball(self, env_id, env_ptr):
        ...
        # self.gym.set_rigid_body_color(env_ptr, ball_handle, 0, gymapi.MESH_VISUAL, gymapi.Vec3(1, 1, 1.))
        ...
        if self.compete_reward:
            ...
            if len(terminate_env_ids > 0):
                ...
                me_reward, op_reward, me_win, op_win = compute_compete_rewards(
                    self.ball_mytable_contact_time_buf[terminate_env_ids, 1], self.ball_othertable_contact_time_buf[terminate_env_ids, 1], self.launch_dir[terminate_env_ids])
...
def compute_compete_rewards(ball_mytable_contact_time_buf, ball_othertable_contact_time_buf, launch_dir):
    me_win = torch.where(launch_dir > 0, ball_othertable_contact_time_buf >= ball_mytable_contact_time_buf, ball_othertable_contact_time_buf > ball_mytable_contact_time_buf)
    op_win = torch.where(launch_dir > 0, ball_othertable_contact_time_buf < ball_mytable_contact_time_buf, ball_othertable_contact_time_buf <= ball_mytable_contact_time_buf)
    both_lose = torch.logical_and(ball_othertable_contact_time_buf == 0, ball_mytable_contact_time_buf == 0)
