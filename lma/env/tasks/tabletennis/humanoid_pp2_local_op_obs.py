...
import torch
import time
import lma.env.tasks.tabletennis.humanoid_pp2 as humanoid_pp2
from lma.env.tasks.tabletennis.humanoid_pp2 import compute_pingpong_observations
...
class HumanoidPP2LocalOpObs(humanoid_pp2.HumanoidPP2):
    ...
    def _compute_task_obs(self, env_ids=None):
        ...
        for i in range(self.num_agents):
            if (env_ids is None):
                ...
                righthand_pos_op = self._rigid_body_pos_list[(i + 1) % 2][:, self._key_body_ids[-2], :].clone()
                righthand_rot_op = self._rigid_body_rot_list[(i + 1) % 2][:, self._key_body_ids[-2], :].clone()
                paddle_pos_op = righthand_pos_op + quat_rotate(righthand_rot_op, self._paddle_to_hand.clone())
                ...
                righthand_pos_op = self._rigid_body_pos_list[(i + 1) % 2][env_ids, self._key_body_ids[-2], :].clone()
                righthand_rot_op = self._rigid_body_rot_list[(i + 1) % 2][env_ids, self._key_body_ids[-2], :].clone()
                paddle_pos_op = righthand_pos_op + quat_rotate(righthand_rot_op, self._paddle_to_hand[env_ids].clone())

            if i==0:
                obs = compute_pingpong_observations(root_states, ball_pos, ball_vel, target_root_pos, paddle_pos, target_ball_pos)
                local_op_obs = compute_opponent_observations(root_states, root_states_op, paddle_pos_op)
...
# @torch.jit.script
def compute_opponent_observations(root_states, root_states_op, paddle_pos_op):
    ...
    local_paddle_pos_op = quat_rotate(heading_rot, paddle_pos_op - root_pos)

    obs = torch.cat([local_pos_op, local_paddle_pos_op],dim=-1)

    return obs

class HumanoidPP2ZLocalOpObs(HumanoidPP2LocalOpObs):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):
