...
class CommonPlayer(players.PpoPlayerContinuous):
    ...
    def __init__(self, config):
        ...
        if (hasattr(self, 'env')):
            ...
            self.num_env = self.env.task.num_envs
        else:
            self.env_num_agents = config["env_info"]["num_agents"]
            self.num_env = config["env_info"]["num_envs"]
        return
    ...
    def run(self):
        ...
        for t in range(n_games):
            ...
            with torch.no_grad():
                for n in range(self.max_steps):
                    ...
                    # if self.env.task.camera_obs:
                    #     self.env.task.get_camera_observation()
                    ...
                    if done_count > 0:
                        ...
                        if self.print_stats:
                            if print_game_res:
                                print('reward:', cur_rewards / done_count_, 'steps:', cur_steps / done_count_, 'w:', game_res)
                            else:
                                print('reward:', cur_rewards / done_count_, 'steps:', cur_steps / done_count_)
        ...
        # # temporal process
        # local_op_obs_buffer = self.local_op_obs_buffer.detach().cpu().numpy()
        # action_buffer = self.action_buffer.detach().cpu().numpy()
        # np.savez('temp.npz', local_op_obs=local_op_obs_buffer, action=action_buffer)

        # temporal process for saving ball position
        if self.env.task.record_ball_pos:
            self.env.task.save_ball_pos()
