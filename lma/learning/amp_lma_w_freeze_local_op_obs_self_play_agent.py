...
class AMPLMAFreezeLocalOpObsSelfPlayAgent(common_agent.CommonAgent):

    def __init__(self, base_name, config):
        self.task_local_op_obs_size = config['task_local_op_obs_size']
        self.expert_num = config.get('expert_num', 1)
        ...
        self.obs_shape = (self.actions_num * self.expert_num + self.task_local_op_obs_size, )
        ...
        if self.normalize_input:
            ...
            self.running_mean_std = RunningMeanStd(self.local_op_obs_shape).to(self.ppo_device)
            self.agent_1_running_mean_std = RunningMeanStd(self.local_op_obs_shape).to(self.ppo_device)
        ...
        #### build opponent model and its optimizer
        net_config = self._build_net_config(adjust_shape=False)
        ...
        # update epoch
        self.epoch_num = config.get("resume_epoch", 0)

        self.reward_info_buf = torch.zeros((0, 1), dtype=torch.float32, device=self.ppo_device)
    ...
    def set_stats_weights(self, weights):
        ...
        if self._normalize_local_op_obs_input:
            if weights['local_op_obs_running_mean_std']['running_mean'].shape == self.local_op_obs_running_mean_std.state_dict()['running_mean'].shape:
                self.local_op_obs_running_mean_std.load_state_dict(weights['local_op_obs_running_mean_std'])
    ...
    def init_tensors(self):
        ...
        self.experience_buffer.tensor_dict['obses'] = torch.zeros(batch_shape + (self.vec_env.env.task.get_self_obs_size() + self.vec_env.env.task.get_task_obs_size(),),
        ...
        if self.expert_num == 1:
            self.experience_buffer.tensor_dict['adjusted_actions'] = torch.zeros(batch_shape + (self.actions_num,),
                                                                dtype=torch.float32, device=self.ppo_device)
        else:
            self.experience_buffer.tensor_dict['adjusted_actions'] = torch.zeros(batch_shape + (self.actions_num - self.expert_num,),
                                                                    dtype=torch.float32, device=self.ppo_device)
        ...
        self.experience_buffer.tensor_dict['local_op_obs'] = torch.zeros(batch_shape + self.local_op_obs_shape,
                                                                dtype=torch.float32, device=self.ppo_device)
    ...
    def get_action_values(self, obs):
        self_task_obs = obs['obs'][..., :-self.task_local_op_obs_size]
        local_op_obs = obs['obs'][..., -self.task_local_op_obs_size:]

        num_actors = self.num_actors * self.env_num_agents // 2

        task_mu, task_sigma = self.vec_env.env.task.get_task_policy_output(self_task_obs)

        if self.update_agent==0:
            local_op_obs_processed = self._preproc_obs(local_op_obs[:num_actors])
            obs_processed = torch.cat([task_mu[:num_actors], local_op_obs_processed], dim=-1)
            local_op_obs_processed_agent_1 = self._preproc_agent_1_obs(local_op_obs[num_actors:])
            obs_processed_agent_1 = torch.cat([task_mu[num_actors:], local_op_obs_processed_agent_1], dim=-1)
            agent_0_input_dict = {
                'is_train': False,
                'prev_actions': None,
                'obs' : obs_processed,
                "obs_orig": obs['obs'][:num_actors],
                'rnn_states' : self.rnn_states[:num_actors] if self.rnn_states is not None else self.rnn_states
            }

            agent_1_input_dict = {
                'is_train': False,
                'prev_actions': None,
                'obs' : obs_processed_agent_1,
                "obs_orig": obs['obs'][num_actors:],
                'rnn_states' : self.rnn_states[num_actors:] if self.rnn_states is not None else self.rnn_states
            }
        elif self.update_agent==1:
            local_op_obs_processed = self._preproc_obs(local_op_obs[num_actors:])
            obs_processed = torch.cat([task_mu[num_actors:], local_op_obs_processed], dim=-1)
            local_op_obs_processed_agent_0 = self._preproc_agent_0_obs(local_op_obs[:num_actors])
            obs_processed_agent_0 = torch.cat([task_mu[:num_actors], local_op_obs_processed_agent_0], dim= -1)
            agent_0_input_dict = {
                'is_train': False,
                'prev_actions': None,
                'obs' : obs_processed_agent_0,
                "obs_orig": obs['obs'][:num_actors],
                'rnn_states' : self.rnn_states[:num_actors] if self.rnn_states is not None else self.rnn_states
            }
        ...
        with torch.no_grad():
            ...
            # NOTE: adjust latent action
            if self.expert_num == 1:
                adjusted_mu_0 = task_mu[:num_actors] + self._lma_action_scale * agent_0_res_dict['actions']
                adjusted_mu_1 = task_mu[num_actors:] + self._lma_action_scale * agent_1_res_dict['actions']
                distr_0 = torch.distributions.Normal(adjusted_mu_0, torch.exp(task_sigma[:num_actors]))
                distr_1 = torch.distributions.Normal(adjusted_mu_1, torch.exp(task_sigma[num_actors:]))
            else:
                # sample experts' latent actions and slightly change it
                latent_dim = task_mu.shape[-1] // self.expert_num
                m_0 = torch.distributions.categorical.Categorical(logits=agent_0_res_dict['actions'][:, :self.expert_num] / torch.sum(agent_0_res_dict['actions'][:, :self.expert_num], dim=-1).unsqueeze(-1))
                selected_expert_0 = m_0.sample()
                m_1 = torch.distributions.categorical.Categorical(logits=agent_1_res_dict['actions'][:, :self.expert_num] / torch.sum(agent_1_res_dict['actions'][:, :self.expert_num], dim=-1).unsqueeze(-1))
                selected_expert_1 = m_1.sample()
                selected_expert_mu_0 = torch.zeros_like(agent_0_res_dict['actions'][:, self.expert_num:]).to(self.device)
                selected_expert_sigma_0 = torch.zeros_like(selected_expert_mu_0).to(self.device)
                selected_expert_mu_1 = torch.zeros_like(selected_expert_mu_0).to(self.device)
                selected_expert_sigma_1 = torch.zeros_like(selected_expert_mu_0).to(self.device)
                task_mu_0 = task_mu[:num_actors]
                task_sigma_0 = task_sigma[:num_actors]
                task_mu_1 = task_mu[num_actors:]
                task_sigma_1 = task_sigma[num_actors:]
                for n in range(self.expert_num):
                    mus_0_n = task_mu_0[selected_expert_0 == n]
                    sigmas_0_n = task_sigma_0[selected_expert_0 == n]
                    selected_expert_mu_0[selected_expert_0 == n] = mus_0_n[:, n * latent_dim: (n + 1) * latent_dim]
                    selected_expert_sigma_0[selected_expert_0 == n] = sigmas_0_n[:, n * latent_dim: (n + 1) * latent_dim]
                    mus_1_n = task_mu_1[selected_expert_1 == n]
                    sigmas_1_n = task_sigma_1[selected_expert_1 == n]
                    selected_expert_mu_1[selected_expert_1 == n] = mus_1_n[:, n * latent_dim: (n + 1) * latent_dim]
                    selected_expert_sigma_1[selected_expert_1 == n] = sigmas_1_n[:, n * latent_dim: (n + 1) * latent_dim]

                adjusted_mu_0 = selected_expert_mu_0 + self._lma_action_scale * agent_0_res_dict['actions'][:, self.expert_num:]
                distr_0 = torch.distributions.Normal(adjusted_mu_0, torch.exp(selected_expert_sigma_0))
                adjusted_mu_1 = selected_expert_mu_1 + self._lma_action_scale * agent_1_res_dict['actions'][:, self.expert_num:]
                distr_1 = torch.distributions.Normal(adjusted_mu_1, torch.exp(selected_expert_sigma_1))

            agent_0_res_dict['adjusted_actions'] = distr_0.sample().squeeze()
            agent_1_res_dict['adjusted_actions'] = distr_1.sample().squeeze()

            if self.has_central_value:
                states = obs['states'][:num_actors]
                agent_0_input_dict = {
                    'is_train': False,
                    'states' : states,
                ...
                ########## oppoent value
                agent_1_states = obs['states'][num_actors:]
                agent_1_input_dict = {
                    'is_train': False,
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            self.experience_buffer.update_data('adjusted_actions', n, res_dict['adjusted_actions'])

            if self.has_central_value:
                self.experience_buffer.update_data('states', n, self.obs['states'])

            self.obs, rewards, self.dones, infos = self.env_step(res_dict['adjusted_actions'])
            # import pdb;pdb.set_trace()
            ...
            # add game_reward_info
            try:
                if self.vec_env.env.task.compete_reward:
                    if self.update_agent == 0:
                        update_done_indices = torch.flatten(done_indices)
                    elif self.update_agent == 1:
                        update_done_indices = torch.flatten(done_indices) + num_actors
                else:
                    update_done_indices = torch.flatten(done_indices)
            except:
                update_done_indices = torch.flatten(done_indices)

            self.reward_info_buf = torch.cat([self.reward_info_buf, self.current_rewards[update_done_indices]], dim=0)
        ...
        if self.update_agent==0: # only states from Agent 0 is used
            for each in self.tensor_list:
                if each=='states': # there are no state in the experience buffer dict.
                    continue
                batch_dict[each] = a2c_common.swap_and_flatten01(self.experience_buffer.tensor_dict[each][:,:num_actors])
            batch_dict['returns'] = a2c_common.swap_and_flatten01(mb_returns[:,:num_actors])
            batch_dict['terminated_flags'] = terminated_flags[:num_actors]
            batch_dict['reward_raw'] =reward_raw / self.horizon_length
            batch_dict['played_frames'] = self.batch_size//2

            for k, v in amp_rewards.items():
                batch_dict[k] = a2c_common.swap_and_flatten01(v[:,:num_actors])
            batch_dict['mb_rewards'] = a2c_common.swap_and_flatten01(mb_rewards[:,:num_actors])
    ...
    def _eval_critic(self, obs_dict):
        self_task_obs = obs_dict['obs'][..., :-self.task_local_op_obs_size]
        local_op_obs = obs_dict['obs'][..., -self.task_local_op_obs_size:]
        task_mu, task_sigma = self.vec_env.env.task.get_task_policy_output(self_task_obs)

        local_op_obs_processed = self._preproc_obs(local_op_obs)
        obs_processed = torch.cat([task_mu, local_op_obs_processed], dim=-1)
        input_dict = {
            'is_train': False,
            'prev_actions': None,
            'obs' : obs_processed,
            "obs_orig": obs_dict['obs'],
            'rnn_states' : self.rnn_states if self.rnn_states is not None else self.rnn_states
        }

        self.model.eval()
        if self.model.is_rnn():
            value, state = self.model.a2c_network.eval_critic(input_dict)
        else:
            value = self.model.a2c_network.eval_critic(input_dict)
    ...
    def prepare_dataset(self, batch_dict):
        ...
        self.dataset.update_values_dict(dataset_dict, rnn_format = True, horizon_length = self.horizon_length, num_envs = self.num_actors * self.env_num_agents // 2)
    ...
    def calc_gradients(self, input_dict):
        ...
        local_op_obs_batch = input_dict['obs'][..., -self.task_local_op_obs_size:]
        task_mu_batch, task_sigma_batch = self.vec_env.env.task.get_task_policy_output(obs_batch[..., :-self.task_local_op_obs_size])
        local_op_obs_processed = self._preproc_obs(local_op_obs_batch, use_temp=self.temp_running_mean)
        obs_batch_processed = torch.cat([task_mu_batch, local_op_obs_processed], dim=-1)
    ...
    def _load_config_params(self, config):
        ...
        self._lma_action_scale = config['lma_action_scale']
    ...
    def _build_net_config(self, adjust_shape=True):
        ...
        if self.expert_num == 1:
            config['input_shape'] = (self.actions_num + self.task_local_op_obs_size, )
        else:
            config['input_shape'] = ((self.actions_num - self.expert_num) * self.expert_num + self.task_local_op_obs_size, )
        ...
        if self.vec_env.env.task.has_task:
            ...
            config['task_obs_size'] = self.vec_env.env.task.get_task_obs_size() - self.task_local_op_obs_size
    ...
    def _assemble_train_info(self, train_info, frame):
        ...
        # add reward info
        if len(self.reward_info_buf):
            reward_std, reward_mean = torch.std_mean(self.reward_info_buf)
            train_info_dict['reward_std'] = reward_std.item()
            train_info_dict['reward_mean'] = reward_mean.item()
            self.reward_info_buf = torch.zeros((0, 1), dtype=torch.float32, device=self.ppo_device)
