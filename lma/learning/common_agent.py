...
class CommonAgent(a2c_continuous.A2CAgent):
    ...
    def __init__(self, base_name, config):
        ...
        self.save_initial_weight = config.get('save_initial_weight', False)
        ...
        while True:
            ...
            if self.rank == 0:
                ...
                if AutoResume is not None and AutoResume.termination_requested():
                    ...
                    self.save(int_model_output_file)
                    cp_path = f"{int_model_output_file}.pth"
                    message = f"[Auto Resume] Terminateing. Epoch: {epoch_num}, Checkpoint: {cp_path}"
                    print(message, flush=True)
                    AutoResume.request_resume(
                        user_dict = {
                            "resume_path": cp_path,
                            "wandb_id" : wandb.run.id if isinstance(wandb.run, wandb.sdk.wandb_run.Run) else None,
                            "resume_epoch": str(epoch_num)
                        },
                        message=message
                    )
                    time.sleep(10)
                    print("Now quitting with status 0")
                    exit(0)
    ...
    def _actor_loss(self, old_action_log_probs_batch, action_log_probs, advantage, curr_e_clip):
        old_action_log_probs_batch = old_action_log_probs_batch.squeeze(-1)
