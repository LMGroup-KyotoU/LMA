...
class AMPResidualSelfPlayAgent(common_agent.CommonAgent):

    def __init__(self, base_name, config):
        self.task_local_op_obs_size = config['task_local_op_obs_size']
        self.expert_num = config.get('expert_num', 1)
        self.origin_action_size = config['origin_action_size']

        assert self.expert_num == 1
    ...
    def get_action_values(self, obs):
        ...
        task_actions = self.vec_env.env.task.get_task_action(self_task_obs)
        ...
        agent_0_res_dict['adjusted_actions'] = task_actions[:num_actors] + agent_0_res_dict['actions']
        agent_1_res_dict['adjusted_actions'] = task_actions[num_actors:] + agent_1_res_dict['actions']
    ...
    def play_steps(self):
        ...
        if self.kl_penalty_w:
            mb_obses = self.experience_buffer.tensor_dict['obses']
            mb_mus = self.experience_buffer.tensor_dict['mus']
            mb_sigmas = self.experience_buffer.tensor_dict['sigmas']
            kl_penalty = self._calc_kl_prior_penalty(mb_obses, mb_mus, mb_sigmas)
            mb_rewards = self._combine_rewards(mb_rewards, amp_rewards, kl_penalty)
        else:
    ...
    def _load_config_params(self, config):
        ...
        #### for KL prior penalty
        self.kl_penalty_w = config.get('kl_penalty_w', 0)
        if self.kl_penalty_w > 0:
            assert self.vec_env.env.task.cfg.env.use_task_model is True
            self.task_local_op_obs_size = config['task_local_op_obs_size']
    ...
    def _build_net_config(self):
        ...
        config['actions_num'] = self.origin_action_size
    ...
    def _combine_rewards(self, task_rewards, amp_rewards, kl_penalty=None):
        ...
        if kl_penalty is not None:
            combined_rewards -= self.kl_penalty_w * kl_penalty
    ...
    def _calc_kl_prior_penalty(self, obs, mus, sigmas):
        B, E, D = obs.shape
        obs_flat = obs.reshape(-1, D)
        mus_flat = mus.reshape(-1, mus.shape[-1])
        sigmas_flat = sigmas.reshape(-1, sigmas.shape[-1])
        # sigmas_flat = torch.exp(sigmas_flat)
        self_task_obs = obs_flat[..., :-self.task_local_op_obs_size]
        prior_mus, prior_sigmas = self.vec_env.env.task.get_task_policy_output(self_task_obs)
        prior_sigmas = torch.exp(prior_sigmas)
        kl = torch_ext.policy_kl(mus_flat, sigmas_flat, prior_mus, prior_sigmas, reduce=False)
        return kl.reshape(B, E, 1)
