...
class AMPLMAHistoryLocalOpObsSelfPlayAgent(common_agent.CommonAgent):

    def __init__(self, base_name, config):
        self.task_local_op_obs_size = config['task_local_op_obs_size']
        self.task_local_op_obs_history_num = config['task_local_op_obs_history_num']
        super().__init__(base_name, config)

        self.switch_frequency = config.get('switch_frequency', 1)
        self.update_agent = config['update_agent']
        self.local_op_obs_shape = (self.task_local_op_obs_size * self.task_local_op_obs_history_num, )
        ...
        if self.normalize_input:
            ...
            self.local_op_obs_running_mean_std = RunningMeanStd(self.local_op_obs_shape).to(self.ppo_device)
            self.agent_1_local_op_obs_running_mean_std = RunningMeanStd(self.local_op_obs_shape).to(self.ppo_device)
        ...
        self.agent_0_local_op_obs_running_mean_std = self.local_op_obs_running_mean_std

        # update epoch
        if "resume_epoch" in config:
            self.epoch_num = config["resume_epoch"]
        else:
            self.epoch_num = 0

        # load pretrained model
        self.base_net_weights = torch_ext.load_checkpoint(config['checkpoint'])
        self.agent_0_model.a2c_network.load_base_net(self.base_net_weights['model'])
        self.agent_1_model.a2c_network.load_base_net(self.base_net_weights['model'])
        self.agent_0_running_mean_std.load_state_dict(self.base_net_weights['running_mean_std'])
        self.agent_1_running_mean_std.load_state_dict(self.base_net_weights['running_mean_std'])

        self.reward_info_buf = torch.zeros((0, 1), dtype=torch.float32, device=self.ppo_device)

        self.local_op_obs_history = torch.zeros((self.env_num_agents * self.num_actors, self.task_local_op_obs_size * self.task_local_op_obs_history_num),
    ...
    def get_full_state_weights_agent_1(self):
        ...
        state['local_op_obs_running_mean_std'] = self.agent_1_local_op_obs_running_mean_std.state_dict()
    ...
    def get_full_state_weights_agent_0(self):
        ...
        state['local_op_obs_running_mean_std'] = self.agent_0_local_op_obs_running_mean_std.state_dict()
    ...
    def init_tensors(self):
        ...
        self.experience_buffer.tensor_dict['obses'] = torch.zeros(batch_shape + (self.obs_shape[0] + self.task_local_op_obs_size,),
                                                                dtype=torch.float32, device=self.ppo_device)
    ...
    def get_action_values(self, obs):
        ...
        local_op_obs = obs['local_op_obs_history']
        ...
        if self.update_agent==0:
            agent_0_input_dict = {
                'is_train': False,
                'prev_actions': None,
                'obs' : self._preproc_obs(obs_orig[:num_actors]),
                'local_op_obs': self._preproc_local_op_obs(local_op_obs[:num_actors]),
                "obs_orig": obs_orig[:num_actors],
            ...
            agent_1_input_dict = {
                'is_train': False,
                'prev_actions': None,
                'obs' : self._preproc_agent_1_obs(obs_orig[num_actors:]),
                'local_op_obs': self._preproc_agent_1_local_op_obs(local_op_obs[num_actors:]),
                "obs_orig": obs_orig[num_actors:],
                'rnn_states' : self.rnn_states[num_actors:] if self.rnn_states is not None else self.rnn_states
            }
        elif self.update_agent==1:
            agent_0_input_dict = {
                'is_train': False,
                'prev_actions': None,
                'obs' : self._preproc_agent_0_obs(obs_orig[:num_actors]),
                'local_op_obs': self._preproc_agent_0_local_op_obs(local_op_obs[:num_actors]),
                "obs_orig": obs_orig[:num_actors],
                'rnn_states' : self.rnn_states[:num_actors] if self.rnn_states is not None else self.rnn_states
            }
    ...
    def play_steps(self):
        ...
        for n in range(self.horizon_length):
            ...
            self.local_op_obs_history[done_indices] = 0
            self.obs["local_op_obs_history"] = self.local_op_obs_history
            self.experience_buffer.update_data('local_op_obs', n, self.obs['local_op_obs_history'])
            ...
            self.local_op_obs_history[:, :-self.task_local_op_obs_size] = self.local_op_obs_history[:, self.task_local_op_obs_size:]
            self.local_op_obs_history[:, -self.task_local_op_obs_size:] = self.obs['obs'][..., -self.task_local_op_obs_size:]
            self.obs["local_op_obs_history"] = self.local_op_obs_history
    ...
    def _eval_critic(self, obs_dict):
        obs_orig = obs_dict['obs'][..., :-self.task_local_op_obs_size]
        local_op_obs = obs_dict['local_op_obs_history']
        # local_op_obs = obs_dict['obs'][..., -self.task_local_op_obs_size:]
        input_dict = {
            'is_train': False,
            'prev_actions': None,
            'obs' : self._preproc_obs(obs_orig),
            'local_op_obs': self._preproc_local_op_obs(local_op_obs),
            "obs_orig": obs_orig,
    ...
    def prepare_dataset(self, batch_dict):
        ...
        dataset_dict['local_op_obs'] = batch_dict['local_op_obs']
    ...
    def update_agent_1_model(self):
        ...
        self.local_op_obs_running_mean_std = self.agent_1_local_op_obs_running_mean_std
        ...
        self.agent_1_local_op_obs_running_mean_std.unfreeze(); self.agent_0_local_op_obs_running_mean_std.freeze()
    ...
    def update_agent_0_model(self):
        ...
        self.local_op_obs_running_mean_std = self.agent_0_local_op_obs_running_mean_std
        ...
        self.agent_0_local_op_obs_running_mean_std.unfreeze(); self.agent_1_local_op_obs_running_mean_std.freeze()
    ...
    def pre_epoch(self, epoch_num):
        ...
        if self._normalize_local_op_obs_input:
            self.local_op_obs_running_mean_std_temp = copy.deepcopy(self.local_op_obs_running_mean_std)
            self.local_op_obs_running_mean_std_temp.freeze()
    ...
    def _preproc_local_op_obs(self, obs_batch, use_temp=False):
        if type(obs_batch) is dict:
            for k, v in obs_batch.items():
                obs_batch[k] = self._preproc_local_op_obs(v, use_temp = use_temp)
        else:
            if obs_batch.dtype == torch.uint8:
                obs_batch = obs_batch.float() / 255.0
        ...
        if self.normalize_input:
            ...
            if use_temp:
                obs_batch_out = self.local_op_obs_running_mean_std_temp(obs_batch_proc)
                obs_batch_orig = self.local_op_obs_running_mean_std(obs_batch_proc)  # running through mean std, but do not use its value. use temp
            else:
    ...
    def _preproc_agent_1_local_op_obs(self, obs_batch, use_temp=False):
        if type(obs_batch) is dict:
            for k, v in obs_batch.items():
                obs_batch[k] = self._preproc_agent_1_local_op_obs(v, use_temp = use_temp)
        else:
            if obs_batch.dtype == torch.uint8:
                obs_batch = obs_batch.float() / 255.0

        if self._normalize_local_op_obs_input:
            obs_batch_proc = obs_batch[:, :self.agent_1_local_op_obs_running_mean_std.mean_size]

            obs_batch_out = self.agent_1_local_op_obs_running_mean_std(obs_batch_proc)  # running through mean std, but do not use its value. use temp

            obs_batch_out = torch.cat([obs_batch_out, obs_batch[:, self.agent_1_local_op_obs_running_mean_std.mean_size:]], dim=-1)
        return obs_batch_out

    def _preproc_agent_0_local_op_obs(self, obs_batch, use_temp=False):
        if type(obs_batch) is dict:
            for k, v in obs_batch.items():
                obs_batch[k] = self._preproc_agent_0_local_op_obs(v, use_temp = use_temp)
        else:
            if obs_batch.dtype == torch.uint8:
                obs_batch = obs_batch.float() / 255.0

        if self.normalize_input:
            obs_batch_proc = obs_batch[:, :self.agent_0_local_op_obs_running_mean_std.mean_size]
            obs_batch_out = self.agent_0_local_op_obs_running_mean_std(obs_batch_proc)  # running through mean std, but do not use its value. use temp

            obs_batch_out = torch.cat([obs_batch_out, obs_batch[:, self.agent_0_local_op_obs_running_mean_std.mean_size:]], dim=-1)
        return obs_batch_out
    ...
    def calc_gradients(self, input_dict):
        ...
        obs_batch = input_dict['obs'][..., :-self.task_local_op_obs_size]
        ...
        local_op_obs_batch = input_dict['local_op_obs']
        # local_op_obs_batch = input_dict['obs'][..., -self.task_local_op_obs_size:]
        local_op_obs_processed = self._preproc_local_op_obs(local_op_obs_batch, use_temp=self.temp_running_mean)
        ...
        if self.only_kin_loss:
            ...
            batch_dict = {'is_train': True, 'amp_steps': self.vec_env.env.task._num_amp_obs_steps, \
                'prev_actions': actions_batch, 'obs': obs_batch_processed, 'local_op_obs': local_op_obs_processed, 'amp_obs': amp_obs, 'amp_obs_replay': amp_obs_replay, 'amp_obs_demo': amp_obs_demo, \
                    "obs_orig": obs_batch
                    }
    ...
    def _build_net_config(self, adjust_shape=True):
        ...
        if self.vec_env.env.task.has_task:
            ...
            config['local_op_obs_size'] = self.task_local_op_obs_size * self.task_local_op_obs_history_num
            if adjust_shape:
