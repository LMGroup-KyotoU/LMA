...
class AMPSEPMCLocalOpObsSelfPlayAgent(common_agent.CommonAgent):
    ...
    def init_tensors(self):
        ...
        self.experience_buffer.tensor_dict['actions'] = torch.zeros(batch_shape + (self.target_action_size,),
        ...
        self.experience_buffer.tensor_dict['adjusted_actions'] = torch.zeros(batch_shape + (self.actions_num,),
                                                                dtype=torch.float32, device=self.ppo_device)
        self.experience_buffer.tensor_dict['mus'] = torch.zeros(batch_shape + (self.target_action_size,),
                                                                dtype=torch.float32, device=self.ppo_device)
        self.experience_buffer.tensor_dict['sigmas'] = torch.zeros(batch_shape + (self.target_action_size,),
                                                                dtype=torch.float32, device=self.ppo_device)
    ...
    def get_action_values(self, obs):
        ...
        if self.update_agent==0:
            obs_processed = self._preproc_obs(obs['obs'][:num_actors])
            obs_processed_agent_1 = self._preproc_agent_1_obs(obs['obs'][num_actors:])
            agent_0_input_dict = {
                'is_train': False,
                'prev_actions': None,
                'obs' : obs_processed,
            ...
            obs_processed = self._preproc_obs(obs['obs'][num_actors:])
            obs_processed_agent_0 = self._preproc_agent_0_obs(obs['obs'][:num_actors])
            agent_0_input_dict = {
                'is_train': False,
                'prev_actions': None,
        ...
        with torch.no_grad():
            ...
            # task_obs[:num_actors, self.target_action] = agent_0_res_dict['actions']
            # task_obs[num_actors:, self.target_action] = agent_1_res_dict['actions']

            task_mu, task_sigma = self.vec_env.env.task.get_task_policy_output(self_obs)
