...
class AMPLMAFreezeLocalOpObsSelfPlayPlayerContinuous(common_player.CommonPlayer):
    ...
    def get_action(self, obs, is_determenistic=False):
        ...
        num_actors = self_task_obs.shape[0] // env_num_agents

        task_mu, task_sigma = self.env.task.get_task_policy_output(self_task_obs)

        agent_0_local_op_obs_processed = self._preproc_obs(local_op_obs[:num_actors])
        agent_0_obs = torch.cat([task_mu[:num_actors], agent_0_local_op_obs_processed], dim=-1)
        agent_1_local_op_obs_processed = self._preproc_obs(local_op_obs[num_actors:])
        agent_1_obs = torch.cat([task_mu[num_actors:], agent_1_local_op_obs_processed], dim=-1)
        ...
        if self.expert_num == 1:
            ...
            task_mu_0 = task_mu[:self.num_actors]
            task_sigma_0 = task_sigma[:self.num_actors]
            task_mu_1 = task_mu[self.num_actors:]
            task_sigma_1 = task_sigma[self.num_actors:]
        ...
        agent_0_res_dict['adjusted_mus'] = adjusted_mu_0
        agent_0_res_dict['adjusted_actions'] = distr_0.sample().squeeze()
        agent_1_res_dict['adjusted_mus'] = adjusted_mu_1
        agent_1_res_dict['adjusted_actions'] = distr_1.sample().squeeze()

        if self.opp_agent == 1:
            mu = agent_0_res_dict['adjusted_mus']
            action = agent_0_res_dict['adjusted_actions']
            opponent_mu = agent_1_res_dict['adjusted_mus']
            ...
            mu = agent_1_res_dict['adjusted_mus']
            action = agent_1_res_dict['adjusted_actions']
            opponent_mu = agent_0_res_dict['adjusted_mus']
    ...
    def _build_net(self, config):
        ...
        if self.normalize_input:
            ...
            obs_shape = (self.task_local_op_obs_size, )
        ...
        if self._normalize_local_op_obs_input:
            self.running_mean_std = RunningMeanStd((self.task_local_op_obs_size, )).to(self.device)
            self.running_mean_std.eval()
        return
